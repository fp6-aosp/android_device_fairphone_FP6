...
#define LOG_TAG "vendor.camx.provider-service"
...
using ::android::hardware::camera::provider::implementation::QTICameraProvider;
...
int main() {
    ALOGI("CameraProvider service is starting.");

    ABinderProcess_setThreadPoolMaxThreadCount(HWBINDER_THREAD_COUNT);

    std::shared_ptr<QTICameraProvider> defaultProvider = ndk::SharedRefBase::make<QTICameraProvider>();
    const std::string serviceName = std::string(QTICameraProvider::descriptor) + "/vendor_qti/0";
